...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData.dark().copyWith( // Use dark theme and set background color to black
      ...
      home: Scaffold(
        appBar: AppBar(
          title: Text(
            'Ultrasonic Radar',
            style: TextStyle(color: Color.fromARGB(255, 31, 71, 3), fontSize: 30), // Change text color to green
          ),
...
class _SensorDataDisplayState extends State<SensorDataDisplay> {
  ...
  void initState() {
    super.initState();
    fetchSensorData();
    timer = Timer.periodic(Duration(seconds: 0), (Timer t) => fetchSensorData());
  }
  ...
  Future<void> fetchSensorData() async {
    const apiUrl = 'https://api.thingspeak.com/channels/2593832/feeds.json?api_key=QMWSV08Z988VHNTV&results=1';
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        CustomPaint(
          size: Size(750, 370), // Width: 900, Height: 500
...
class ProtractorPainter extends CustomPainter {
  ...
  final Paint _lightGreenPaint = Paint()
  ...
  ProtractorPainter({required this.ultrasonicDistance, required this.angleValue});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i <= 4; i++) {
      TextPainter distancePainter = TextPainter(
        text: TextSpan(
          text: '${i * 5}cm',
          style: TextStyle(color: Color.fromARGB(255, 3, 117, 22), fontSize: 18),
        ),
        textDirection: TextDirection.ltr,
      );
      distancePainter.layout();
      double distanceX = radius - (i * 100);
    ...
    for (int i = 1; i <= 4; i++) {
      double innerRadius = radius - (i * 90);
    ...
    for (int i = 0; i <= 18; i++) {
      ...
      canvas.drawLine(
        center,
        Offset(x, y),
        _paint..color = Color.fromARGB(255, 92, 173, 0),
      );
      ...
      textPainter.layout();
      double labelX = radius + (radius - 20) * math.cos(angle);
      double labelY = radius + (radius - 20) * math.sin(angle);
  ...
  void drawDistanceBox(Canvas canvas, double angle, double radius, Paint paint, int i) {
    // Calculate coordinates for placing the distance box
    double boxCenterX = radius + (radius - (-40)) * math.cos(angle);
    double boxCenterY = radius + (radius - (-30)) * math.sin(angle);
    ...
    if (i * 10 == currentAngle) {
      ...
      TextPainter textPainter = TextPainter(
        text: TextSpan(
          ...
          style: TextStyle(color: const Color.fromARGB(255, 0, 0, 0), fontSize: 14), // Adjust font size
        ),
        textDirection: TextDirection.ltr,
      );
      textPainter.layout();
